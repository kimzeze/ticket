...
function buy() {
  ...
  seatsLength = seats.length; // 여기서 값을 업데이트합니다.
...
function checkout_count() {
  waitForElementToDisplay('select[name="SeatCount"]', 100, function () {
    ...
    // seatsLength 전역 변수를 사용합니다.
    if (seatsLength >= 1 && seatsLength <= 4) {
      selectElement.value = String(seatsLength);
    } else if (seatsLength > 4) {
